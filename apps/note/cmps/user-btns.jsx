...
export class UserBtns extends React.Component {
    ...
    onDuplicateNote = () => {
        ...
        if (type === 'note-video' || type === 'note-img') {
            noteService.createNote(type, backgroundColor,note.info.url)
                .then(note => {
                    this.props.onAddNote(note)
                })
        } else if (type === 'note-todos') {
            noteService.createNote(type, backgroundColor,note.info.title,'', note.info.todos)
                .then(note => {
                    this.props.onAddNote(note)
                })
        } else {
            console.log('note:', note)
            noteService.createNote(type, backgroundColor,note.info.txt)
                .then(note => {
                    this.props.onAddNote(note)
                })
        }
    ...
    render() {
        ...
        return <section className="user-btns">
            ...
            {isColorOn && <div className="color-pick">
                <div className="picked-color" style={{ backgroundColor: 'rgb(237 122 122)' }} onClick={onGetColor}></div>
                ...
                <div className="picked-color" style={{ backgroundColor: 'rgb(245 236 77)' }} onClick={onGetColor}></div>
            </div>}
        </section>
    }
}